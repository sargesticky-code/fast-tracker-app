  ...
  return (
    ...
    >
      <div className="ft5-live-grid">
        ...
        <div className="ft5-live-control">
          {shadow ? (
            <>
              <div><span>PRE</span><b>{shadowSideLabel(match, shadow.expectedSide)}</b></div>
              <div><span>LIVE CTRL</span><b>{shadowSideLabel(match, shadow.actualSide)}</b></div>
        ...
        <div className="ft5-live-stats">
          {showStats ? (
            <>
              <div><span>xG</span><b>{statPairText(stats.xg, 2)}</b></div>
              <div><span>射門</span><b>{statPairText(stats.shots)}</b></div>
              <div><span>中框</span><b>{statPairText(stats.shotsOnTarget)}</b></div>
...
function TopBetCard({ row, index = 0, changeType = null }) {
  ...
  const p = Number.isFinite(probability) ? Math.round(probability * 100) + "%" : "—";
    ...
    >
      <div className="ft5-topbet-grid">
        ...
        <div className="ft5-topbet-fixture">
          <div><span>{formatKickoff(match.kickoff)}</span><small>{match.league}</small></div>
          <b>{match.homeZh || match.home} <em>vs</em> {match.awayZh || match.away}</b>
        </div>
        <div className="ft5-topbet-source">
          <b>{modelSourceLabel(match)}</b>
          <div className="ft5-topbet-meta-line"><span>{type}</span><small className={"ft5-consensus ft5-consensus-" + agreement.key}>{agreement.label}</small></div>
        </div>
...
export default function DashboardClient({ feed, nowMs }) {
  ...
  const headings = {
    focus: ["NEXT 24H", "按 Review Priority 排序"],
