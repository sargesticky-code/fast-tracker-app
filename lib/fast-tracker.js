...
export function goalsValueEdge(match) {
  if (!sameLine(match.goals?.line, 2.5)) return null;
  const model = match.forebetDetail?.ou25;
  const market = binaryFair(match.goals?.over, match.goals?.under);
  if (!model || !market || model.over == null || model.under == null) return null;
  const diffs = [
    { key: "O", value: Number(model.over) - market.over },
    { key: "U", value: Number(model.under) - market.under },
  ];
  return diffs.sort((a, b) => b.value - a.value)[0];
}

export function cornersValueEdge(match) {
  if (!sameLine(match.corners?.line, 9.5)) return null;
  const model = match.forebetDetail?.corners95;
...
export function lineComparisonStatus(match, market) {
  ...
  const target = isGoals ? 2.5 : 9.5;
  const currentRaw = isGoals ? match.goals?.line : match.corners?.line;
  const current = Number(currentRaw);
  const model = isGoals ? match.forebetDetail?.ou25 : match.forebetDetail?.corners95;
  const hasModel = Boolean(model && (model.over != null || model.under != null || model.avgGoals != null || model.avgCorners != null));

  if (!hasModel) {
    return { key: "missing", comparable: false, currentLine: Number.isFinite(current) ? current : null, modelLine: target, label: "NO DATA" };
  }
  if (!Number.isFinite(current)) {
    return { key: "no-market", comparable: false, currentLine: null, modelLine: target, label: "HKJC NO LINE" };
  }
  if (!sameLine(current, target)) {
    return {
      key: "mismatch",
      comparable: false,
      currentLine: current,
      modelLine: target,
      label: `FB ${target} ≠ HKJC ${current}`,
    };
  }
  return { key: "comparable", comparable: true, currentLine: current, modelLine: target, label: "可直接比較" };
}
