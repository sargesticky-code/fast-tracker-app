...
export async function getFeed() {
  try {
    // Railway is the production frontend and must render the latest Supabase
    // state on every navigation. Static Cloudflare builds may cache only at
    // build time; the client refresh lane takes over after hydration.
    const staticBuild = process.env.CF_PAGES === "1" || process.env.CF_PAGES === "true" || process.env.GITHUB_ACTIONS === "true";
    const res = await fetch(FEED_URL, staticBuild ? { cache: "force-cache" } : { cache: "no-store" });
