...
async function resolve(source:string,name:unknown,competition:unknown,eventId:unknown,side:"HOME"|"AWAY"){const n=text(name);if(!n)return null;const {data,error}=await db.rpc("ft_resolve_team_name_context",{p_source:source,p_name:n,p_source_competition:text(competition),p_hkjc_event_id:text(eventId),p_side:side});if(error)throw new Error(`identity ${source}/${n}: ${error.message}`);return data?.[0]||null}
async function identityTelemetry(source:string,rows:Record<string,string>[],spec:{event:string,competition?:string,home:string,away:string}){const counts:Record<string,number>={EVENT_CONTEXT:0,LEAGUE_CONTEXT:0,SOURCE_MASTER:0,GLOBAL_UNIQUE:0,NONE:0};for(const r of rows){for(const [field,side] of [[spec.home,"HOME"],[spec.away,"AWAY"]] as const){const x=await resolve(source,r[field],spec.competition?r[spec.competition]:null,r[spec.event],side);const p=String(x?.resolution_path||"NONE");counts[p]=(counts[p]||0)+1}}return counts}
async function telemetryAssets(){const specs=[
["FOOTBALL_DATA","football_data_current.csv","hkjc_event_id","league","home_team","away_team"],
["FORM","form_current.csv","hkjc_event_id","league","home_team","away_team"],
